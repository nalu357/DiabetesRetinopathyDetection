...
def get_bunet(input_img, n_filters=16, dropout=1.0, batchnorm=True, training =True):
    # contracting path
...
def get_deep_bunet(input_img, n_filters=16, dropout=1.0, batchnorm=True, training =True):
    # contracting path
    c1 = conv2d_block(input_img, n_filters=n_filters * 1, kernel_size=3, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2))(c1)
    p1 = Dropout(dropout)(p1, training=training)
    ...
    p7 = Dropout(dropout)(p7, training=training)

    c8 = conv2d_block(p7, n_filters=n_filters * 128, kernel_size=3, batchnorm=batchnorm)

    # expansive path
    u8 = Conv2DTranspose(n_filters * 64, (3, 3), strides=(2, 2), padding='same')(c8)
    u8 = concatenate([u8, c7])
    u8 = Dropout(dropout)(u8, training=training)
    c9 = conv2d_block(u8, n_filters=n_filters * 64, kernel_size=3, batchnorm=batchnorm)

    u9 = Conv2DTranspose(n_filters *32, (3, 3), strides=(2, 2), padding='same')(c9)
    u9 = concatenate([u9, c6])
    u9 = Dropout(dropout)(u9, training=training)
    c10 = conv2d_block(u9, n_filters=n_filters * 32, kernel_size=3, batchnorm=batchnorm)

    u10 = Conv2DTranspose(n_filters *16, (3, 3), strides=(2, 2), padding='same')(c10)
    u10 = concatenate([u10, c5])
    u10  = Dropout(dropout)(u10, training=training)
    c11 = conv2d_block(u10, n_filters=n_filters * 16, kernel_size=3, batchnorm=batchnorm)

    u11 = Conv2DTranspose(n_filters *8, (3, 3), strides=(2, 2), padding='same')(c11)
    u11 = concatenate([u11, c4])
    u11 = Dropout(dropout)(u11, training=training)
    c12 = conv2d_block(u11, n_filters=n_filters * 8, kernel_size=3, batchnorm=batchnorm)

    u12 = Conv2DTranspose(n_filters * 4, (3, 3), strides=(2, 2), padding='same')(c12)
    u12 = concatenate([u12, c3])
    u12 = Dropout(dropout)(u12, training=training)
    c13 = conv2d_block(u12, n_filters=n_filters * 4, kernel_size=3, batchnorm=batchnorm)

    u13 = Conv2DTranspose(n_filters * 2, (3, 3), strides=(2, 2), padding='same')(c13)
    u13 = concatenate([u13, c2])
    u13 = Dropout(dropout)(u13, training=training)
    c14 = conv2d_block(u13, n_filters=n_filters * 2, kernel_size=3, batchnorm=batchnorm)

    u14 = Conv2DTranspose(n_filters * 1, (3, 3), strides=(2, 2), padding='same')(c14)
    u14 = concatenate([u14, c1])
    u14 = Dropout(dropout)(u14, training=training)
    c15 = conv2d_block(u14, n_filters=n_filters * 1, kernel_size=3, batchnorm=batchnorm)

    prediction = Conv2D(1, (1, 1))(c15)
    variances = Conv2D(1, (1, 1), activation="softplus")(c15)
...
def bayes_bunet(input_img, n_filters=16, dropout=1.0, batchnorm=True, training =True):
    # contracting path
    c1 = conv2d_block(input_img, n_filters=n_filters * 1, kernel_size=3, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2))(c1)
    p1 = Dropout(dropout)(p1, training=training)
...
def bayes_bunet1(input_img, n_filters=16, dropout=1.0, batchnorm=True, training =True):
    # contracting path
    c1 = conv2d_block(input_img, n_filters=n_filters * 1, kernel_size=3, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2))(c1)
    p1 = Dropout(dropout)(p1, training=training)
